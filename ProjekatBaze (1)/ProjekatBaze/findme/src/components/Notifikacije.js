...
export default function Notifikacije(props){
    ...
    const closeNotifications=(e)=>{
        if(showNotifikacije && ref.current && !ref.current.contains(e.target) && refBell.current && !refBell.current.contains(e.target))
    ...
    useEffect(()=>{
        if(props.novaNotifikacija && props.novaNotifikacija.text)
        {
    ...
    const obrisiNotifikacije=async()=>{
        const response=await fetch("http://localhost:5000/api/Del/obrisiNotifikacije/"+localStorage.getItem("username"),  {method:"DELETE",
    ...
    return(
      <>
      <div >
          <svg ref={refBell} onClick={()=>setShownotifikacije(prev=>!prev)} xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-bell ikonicaBar" viewBox="0 0 16 16">
        ...
        {showNotifikacije &&  <div ref ={ref} className='notifikacije'  style={{position:'absolute',overflow:'auto',right:'-100px',top:'50px'}}>
              ...
              <div className='delNots' onClick={obrisiNotifikacije}>{'<Obrisi sve notifikacije>'}</div>
              {(!notifikacije || notifikacije.length===0) && <div className='nemaNots'>Trenutno nemate notifikacija. </div>}
